...
derivMoni var (Moni coef vars ) | null filteredVars = Moni 0 [('_',0)] --Monomial that doesn't have the variable var
                                | filteredVarsAux == [] = Moni coefNew varsAux
                                | otherwise = Moni coefNew filteredVarsAux
                                 where filteredVars = filter (\(y,z) ->  y == var) vars --filtered list of Vars that only include elements
                                       ...
                                       filteredVarsAux = filter (\(y,z) -> z /= 0) varsAux --Removes variables that have degree 0
...
derivPoli x [] = []
derivPoli x l = normalizePoli [derivMoni x y | y <- l]
